...
/** Number of skeleton/health tiles. */
const TILE_COUNT = 4
...
/** The error card shown when the aggregate query fails. */
...
/** The skeleton tiles shown while the series loads. */
...
/** The action-oriented empty state for a window with no delivery events. */
...
/** The four delivery-health tiles for a populated window. */
function HealthTiles({ rows }: { rows: AggregateBucket[] }) {
  const sent = totalOf(rows, 'sent')
  const failed = totalOf(rows, 'failed')
  const deliveryRate = ratePct(sent, sent + failed)
  const verifyRate = ratePct(totalOf(rows, 'verified'), totalOf(rows, 'generated'))
  return (
    <div className="flex flex-wrap gap-4">
      <StatTile
        title="DELIVERED"
 ...
 * @returns The four-tile health row, a skeleton, an error, or an empty state.
 */
export function HealthStrip({ query }: HealthStripProps) {
  const { data, isLoading, isError } = useAggregate('verb', query)
  if (isError) return <HealthError />
  if (isLoading) return <HealthSkeleton />
  const rows = data ?? []
  if (grandTotal(rows) === 0) return <HealthEmpty />
  return <HealthTiles rows={rows} />
}
